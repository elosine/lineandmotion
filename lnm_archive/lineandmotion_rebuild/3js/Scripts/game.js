...
function createScene() {
  ...
  // camera.position.x = -(fieldLength / 2) - 47;
  ...
  //Tempo Frets
  fgeom = new THREE.CubeGeometry(4, 194, 5);
  fmatl = new THREE.MeshBasicMaterial({
    color: "rgb(255,255,0)"
  });
  fret = new THREE.Mesh(fgeom, fmatl);
  scene.add(fret);
  var fposx = -340;
  var fposz = 3;
  fret.position.x = fposx;
  fret.position.z = fposz;

  // wireframe
  var geo = new THREE.EdgesGeometry(fgeom);
  var mat = new THREE.LineBasicMaterial({
    color: 0x000000,
    linewidth: 4
  });
  var wf = new THREE.LineSegments(geo, mat);
  wf.renderOrder = 1; // make sure wireframes are rendered 2nd
  scene.add(wf);
  wf.position.x = fposx;
  wf.position.z = fposz;

  //Add Red fret and move
